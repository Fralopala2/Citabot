...
fro                try:
                    ...
                    # Removed verbose Brotli logging
                except ImportError:
                    response_text = response.text
                    # Removed verbose fallback logging
                except Exception as e:
                    response_text = response.text
                    # Only log critical decompression errors
                    if response.status_code != 200:
                        print(f"⚠️ Brotli decompression failed for {module}: {e}")rt Dict, List, Optional, Any
...
class SitValScraper:
    ...
    def _make_request(self, module: str, payload: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # Only log important status changes, not every request
            if response.status_code != 200:
                print(f"⚠️ Error: {module} returned {response.status_code}")
